...
feed_sparql() / (MST_ACT(X, Y, Z, T) & MST_VAR(Y, "Where01:WRB") & MST_VAR(Z, W) & MST_VAR(T, "?")) >> [show_line("\nWHERE (active) question detected..."), -MST_ACT(X, Y, Z, T), -MST_VAR(Y, "Where01:WRB"), -MST_VAR(Z, W), -MST_VAR(T, "?"), feed_query_sparql(X, Y), feed_sparql()]
# Passive (Where Colonel West was born?)
feed_sparql() / (MST_ACT(X, Y, Z, T) & MST_VAR(Y, "Where01:WRB") & MST_VAR(Z, "?") & MST_VAR(T, K)) >> [show_line("\nWHERE (passive) question detected..."), -MST_ACT(X, Y, Z, T), -MST_VAR(Y, "Where01:WRB"), -MST_VAR(Z, K), -MST_VAR(T, "?"), feed_query_sparql(X, Y), feed_sparql()]
...
feed_sparql() / (MST_ACT("Be01:VBZ", X, Z, T) & MST_VAR(Z, W) & MST_VAR(T, K)) >> [show_line("\nPOLAR (copular) question detected..."), -MST_ACT("Be01:VBZ", X, Z, T), -MST_VAR(Z, W), -MST_VAR(T, K), feed_query_sparql(X, Y), feed_sparql()]
