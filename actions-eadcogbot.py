...
class process_clause_final(Procedure): pass
...
class handle_msg(Procedure): pass
...
class llm_get_last_shell(Action):
    """get LLM result from shell"""
...
class llm_get(Action):
    ...
    def execute(self, *args):
        a = str(args).split("'")[5]
        result = parser.get_LLM(a)
        self.assert_belief(OUT(result))

class llm_get_last(Action):
    """get LLM result"""
    def execute(self, *args):
        a = str(args).split("'")[5]
        result = parser.get_LLM(a)
...
class llm_fol_get(Action):
    """get LLM result fromm FOL"""
    ...
    def execute(self, *args):
        ...
        longest_item = max(key_value_pairs, key=len)

        if len(longest_item) > 5:
            ...
            self.assert_belief(OUT(longest_item))
        else:
            pass # far dire qualcosa nel caso False
...
class reason(Action):
    ...
    def execute(self, *args):
        ...
        if bc_result is not False:
            print("\nFrom HKB (Backward-Chaining): True")
            text_from_llm = parser.get_LLM_from_fol(str(bc_result))
            print(text_from_llm) # shell
            self.assert_belief(OUT(text_from_llm))
            self.assert_belief(ANSWERED("YES"))
        else:
            print("OUTPUT:", str(bc_result))
            print("\nFrom HKB (Backward-Chaining): False")
        ...
        if bc_result is False and NESTED_REASONING:
            ...
            if nested_result is False:
                print("\nFrom HKB (Nested Reasoning): ", nested_result)

            else:
                print("\nResult: ", nested_result)
                print("\nFrom HKB (Nested Reasoning): True")
                text_from_llm = parser.get_LLM_from_fol(str(nested_result))
                print(text_from_llm)  # shell
                self.assert_belief(OUT(text_from_llm))
                self.assert_belief(ANSWERED("YES"))
        ...
        if LKB_USAGE and bc_result is False and nested_result is False:
            ...
            if bc_result is not False:
                print("\nFrom LKB (Backward-Chaining): True")
                text_from_llm = parser.get_LLM_from_fol(str(bc_result))
                print(text_from_llm)  # shell
                self.assert_belief(OUT(text_from_llm))
                self.assert_belief(ANSWERED("YES"))
                ...
                if nested_result is False:
                    print("\nResult: ", nested_result)
                    print("\nFrom LKB (Nested Reasoning): False")

                else:
                    print("\nResult: ", nested_result)
                    print("\nFrom LKB (Nested Reasoning): True")
                    text_from_llm = parser.get_LLM_from_fol(str(nested_result))
                    self.assert_belief(OUT(text_from_llm))
                    self.assert_belief(ANSWERED("YES"))
            else:
                print("\nFrom LKB (Backward-Chaining): False")
...
class wfc(ActiveBelief):
    def evaluate(self, y):
        s = str(y).replace("'Variable'", "")
